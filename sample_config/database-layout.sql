...
FROM
  (
     (SELECT timed_thinclient_to_vm_mapping.id,
      ...
      FROM timed_thinclient_to_vm_mapping
      WHERE ((timed_thinclient_to_vm_mapping.start_date IS NULL)
             AND (timed_thinclient_to_vm_mapping.end_date IS NULL))
      UNION SELECT timed_thinclient_to_vm_mapping.id,
                   timed_thinclient_to_vm_mapping.thinclient,
                   timed_thinclient_to_vm_mapping.vm,
                   timed_thinclient_to_vm_mapping.resolution,
                   timed_thinclient_to_vm_mapping.start_date,
                   ...
                   timed_thinclient_to_vm_mapping.shutdown_vm
      FROM timed_thinclient_to_vm_mapping
      WHERE ((timed_thinclient_to_vm_mapping.start_date <= now())
             AND (timed_thinclient_to_vm_mapping.end_date IS NULL)))
   UNION SELECT timed_thinclient_to_vm_mapping.id,
                timed_thinclient_to_vm_mapping.thinclient,
                timed_thinclient_to_vm_mapping.vm,
                ...
                timed_thinclient_to_vm_mapping.shutdown_vm
   FROM timed_thinclient_to_vm_mapping
   WHERE ((now() >= timed_thinclient_to_vm_mapping.start_date)
          AND (now() <= timed_thinclient_to_vm_mapping.end_date))) f
ORDER BY f.prio DESC,
         f.start_date,
         f.id;
...
SELECT DISTINCT (((timed_thinclient_to_vm_mapping.thinclient)::text || '.thinclients.yourdomain.site'::text))::character varying AS dhcp_hostname,
                timed_thinclient_to_vm_mapping.thinclient
FROM timed_thinclient_to_vm_mapping
UNION
SELECT DISTINCT timed_thinclient_to_vm_mapping.thinclient AS dhcp_hostname,
                timed_thinclient_to_vm_mapping.thinclient
FROM timed_thinclient_to_vm_mapping;
...
CREATE TABLE dhcphostname_to_thinclient_mapping (
	dhcp_hostname character varying NOT NULL,                                                                          thinclient character varying NOT NULL);
...
SELECT f.dhcp_hostname,
       f.systemuuid,
       f.thinclient,
       f.prio
FROM
  (
     (SELECT dhcphostname_to_thinclient_auto_mapping.dhcp_hostname,
             NULL::character varying AS systemuuid,
             dhcphostname_to_thinclient_auto_mapping.thinclient,
             100 AS prio
      FROM dhcphostname_to_thinclient_auto_mapping
      UNION SELECT dhcphostname_to_thinclient_mapping.dhcp_hostname,
                   NULL::character varying AS systemuuid,
                   dhcphostname_to_thinclient_mapping.thinclient,
                   200 AS prio
      FROM dhcphostname_to_thinclient_mapping)
   UNION SELECT NULL::character varying AS dhcp_hostname,
                systemuuid_to_thinclient_mapping.systemuuid,
                systemuuid_to_thinclient_mapping.thinclient,
                300 AS prio
   FROM systemuuid_to_thinclient_mapping) f
ORDER BY f.prio DESC;
